...
// 转换schema到table
export function transformSchemaToTable(schema: any) {
  try {
...
//  自动添加type
function checkJsonSchema(json: any) {
...
const mapping: Function = function mapping(data: any, index: string | number) {
  switch (data.type) {
    case 'string':
...
const SchemaObject = (data: any, key: string | number) => {
...
const SchemaArray = (data: any, index: string | number) => {
